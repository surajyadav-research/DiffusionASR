...
class DeformConv1d(nn.Module):
    ...
    def forward(
        ...
        if self.padding == "same":
            assert (
                in_shape[-1] == output.shape[-1]
            ), f"input length {in_shape} and output length {output.shape} do not match."
        return output
...
class PackedDeformConv1d(DeformConv1d):
    ...
    def forward(self, input, with_offsets=False):
        ...
        offsets = self.odp_norm(self.odp_prelu(offsets).moveaxis(1, 2)).moveaxis(
            2, 1
